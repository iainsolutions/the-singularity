#!/usr/bin/env python3
"""
Scenario Test: Corporations (age 8, green)

Effects:
- Effect 0 (DEMAND): Transfer a top non-green card with factory from your board to my score.
  Primitives: DemandEffect, FilterCards (board_top, not green, has factory), SelectCards, TransferBetweenPlayers
- Effect 1: Draw and meld an 8.
  Primitives: DrawCards (age 8), MeldCard

Setup:
- Human: Corporations on board (factory symbols)
...
class TestCorporationsScenario:
    """Test Corporations demand + draw and meld."""
    ...
    def setup_scenario(self) -> dict[str, Any]:
        ...
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Corporations", "location": "board"})
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
    ...
    def test_corporations_demand_and_meld(self):
        """Test Corporations: demand skipped, draw and meld 8."""
        ...
        response = requests.post(
            ...
            json={"player_id": human_id, "action_type": "dogma", "card_name": "Corporations"}
        )
        assert response.status_code == 200
        time.sleep(2)
        ...
        print("✅ Corporations test PASSED")
