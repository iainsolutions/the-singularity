#!/usr/bin/env python3
"""
Scenario Test: Mobility (age 8, red)

Effects:
- Effect 0 (DEMAND): Transfer your two highest non-red top cards without factory to my score pile.
  Primitives: DemandEffect, FilterCards, SelectHighest (different_colors), TransferBetweenPlayers

Setup:
- Human: Mobility on board (factory symbols)
...
class TestMobilityScenario:
    ...
    def setup_scenario(self) -> dict[str, Any]:
        ...
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Mobility", "location": "board"})
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
    ...
    def test_mobility_demand_skipped(self):
        """Test Mobility: demand skipped (AI has fewer factories)."""
        ...
        response = requests.post(
            ...
            json={"player_id": human_id, "action_type": "dogma", "card_name": "Mobility"}
        )
        assert response.status_code == 200
        time.sleep(2)
