#!/usr/bin/env python3
"""
Scenario Test: Reclamation (age 11, yellow)

Effects:
- Effect 0: Return your three bottom red cards. Draw and meld a card of value equal to
  half the total sum value of the returned cards, rounded up. If you return three cards,
  repeat this effect using the color of the next color clockwise.

Setup: Human has Reclamation on board + some red cards for returning.
"""
...
class TestReclamationScenario:
    def setup_scenario(self) -> dict[str, Any]:
        ...
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Reclamation", "location": "board"})
        # Add a red card to have something to return
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Archery", "location": "board"})
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
