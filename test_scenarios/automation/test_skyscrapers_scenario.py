#!/usr/bin/env python3
"""
Scenario Test: Skyscrapers (age 8, yellow)

Effects:
- Effect 0 (DEMAND): Transfer a top non-yellow card with crown from opponent's board to mine.
  Primitives: DemandEffect, FilterCards, SelectCards, TransferBetweenPlayers

Setup:
- Human: Skyscrapers on board (crown symbols)
- AI: Agriculture (0 crowns) - demand skipped
"""
...
class TestSkyscrapersScenario:
    """Test Skyscrapers demand transfer."""
    ...
    def setup_scenario(self) -> dict[str, Any]:
        ...
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Skyscrapers", "location": "board"})
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
    ...
    def test_skyscrapers_demand_skipped(self):
        """Test Skyscrapers: demand skipped (AI has fewer crowns)."""
        ...
        response = requests.post(
            ...
            json={"player_id": human_id, "action_type": "dogma", "card_name": "Skyscrapers"}
        )
        assert response.status_code == 200
        time.sleep(2)
