#!/usr/bin/env python3
"""
Scenario Test: Explosives (age 7, red)

Effects:
- Effect 0 (DEMAND): Transfer the three highest cards from your hand to my hand.
  Primitives: DemandEffect, SelectHighest (hand, 3), TransferBetweenPlayers

Setup:
- Human: Explosives on board (3 factories)
- AI: Agriculture on board (0 factories) - demand skipped
"""
...
class TestExplosivesScenario:
    """Test Explosives demand transfer."""
    ...
    def setup_scenario(self) -> dict[str, Any]:
        ...
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
                      json={"player_id": human_id, "card_name": "Explosives", "location": "board"})
        requests.post(f"{BASE_URL}/admin/dev/admin/games/{game_id}/set-card",
    ...
    def test_explosives_demand_skipped(self):
        """Test Explosives: demand skipped (AI has fewer factories)."""
        ...
        response = requests.post(
            ...
            json={"player_id": human_id, "action_type": "dogma", "card_name": "Explosives"}
        )
        assert response.status_code == 200
        time.sleep(2)
